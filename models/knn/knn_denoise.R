...
train_seas_filtered <- NoiseFiltersR::IPF(seasonal_vaccine ~ ., train_seas, class_column=seasonal_vaccine) 
train_seas <- train_seas_filtered$cleanData
...
knn_woe_denoise <- knn_seas_wf %>% tune_grid(
  resamples=kfolds,
  grid = knn_grid
)


knn_woe_denoise_res <- knn_woe_res_more_vars %>% collect_metrics()%>% mutate(familia="knn_woe_denoise",
                                                                                 target="seasonal_vaccine")
knn_woe_rs
read_csv("./models_cv_results.csv") %>% bind_rows(knn_woe_denoise_res)%>%
...
knn_woe_h1n1_res <- knn_woe_h1n1 %>% collect_metrics()  %>% mutate(familia="knn_woe",
                                                                   target="h1n1_vaccine")
