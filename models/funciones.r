...
leer_datos <- function(ruta_predictores_train, ruta_etiquetas_train,
                       ruta_predictores_test, juntar_etiquetas = FALSE,
                       factores_ordenados= c(1,2,16:23,26),
                       factores = c(3:15, 24:25, 29:32)) {
    ...
    x_train <- x_train %>% mutate(across(all_of(factores_ordenados), as.ordered))
    x_test <- x_test %>% mutate(across(all_of(factores_ordenados), as.ordered))
    
    ##El resto de variables, menos household_adults y household_children las pasamos
    ##a factor sin orden.
    x_train <- x_train %>% mutate(across(all_of(factores), as.factor))
    x_test <- x_test %>% mutate(across(all_of(factores), as.factor))
