...
##
# Esta función evalúa un modelo de naive bayes con los datos proporcionados
# Solo sirve para datos con las etiquetas h1n1 vaccine y seasonal vaccine.
# Devuelve el AUC y la precisión media
##
cv_train_naive_bayes<-function(x_train,y_train,laplace=0,k_folds=10){
  
  # Esta función auxiliar entrena naive bayes en un fold de CV
  fold_naive_bayes<-function(fold,data,labels,label_pos,laplace=0){
    #Separo train y test según la partición de la validación cruzada.
    ...
    #Calculo la predicción de clases para accuracy
    pred <- predict(nb,test_fold)
    accuracy <- sum(pred==test_fold[,pred_label])/length(fold)
    
    #Calculo la probabilidad de pertenencia a cada clase según naive bayes para AUC
...
## Después de feature selection (en naive_vayes_FS.R)
...
#Ordena las variables según el cambio de probabilidad de las clases según naive bayes.
analizar_informacion_naive_bayes<-function(modelo_nb,umbral=0){
  ...
  list(informacion[ordenados],informacion_min[ordenados_min])
}
...
###
# Pequeño análisis de resultados confusos
###

# Muestro los resultados que están en torno a 0.5.
...
pred_sin_problematicos<-predict(nb_h1n1,h1n1_train[-problematicos_h1n1,])

sum(pred_sin_problematicos!=y_train[-problematicos_h1n1,]$h1n1_vaccine)/length(y_train[-problematicos_h1n1,]$h1n1_vaccine)
